...
namespace Saro.XAsset.Build
{
    internal class AssetTreeView : TreeViewWithTreeModel<AssetTreeElement>
    {
        ...
        static Dictionary<Type, Texture2D> s_TestIcons = new Dictionary<Type, Texture2D>
        {
            { typeof(SceneAsset) , EditorGUIUtility.FindTexture("SceneAsset Icon")},
            { typeof(GameObject), EditorGUIUtility.FindTexture("Prefab Icon")},
        };
            ...
            for (int i = 1; i < sortedColumns.Length; i++)
            {
                ...
                switch (sortOption)
                {
                    ...
                    case SortOption.Size:
                        orderedQuery = orderedQuery.ThenBy(l => l.data.asset.len, ascending);
                        break;
                }
            }
        ...
        IOrderedEnumerable<TreeViewItem<AssetTreeElement>> InitialOrder(IEnumerable<TreeViewItem<AssetTreeElement>> myTypes, int[] history)
        {
            ...
            switch (sortOption)
            {
                ...
                case SortOption.Size:
                    return myTypes.Order(l => l.data.asset.len, ascending);
                default:
        ...
        int GetIcon(TreeViewItem<AssetTreeElement> item)
        {
        ...
        void CellGUI(Rect cellRect, TreeViewItem<AssetTreeElement> item, MyColumns column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case MyColumns.Icon:
                    {
                        //GUI.DrawTexture(cellRect, s_TestIcons[GetIcon(item)], ScaleMode.ScaleToFit);
                    }
                    break;
                ...
                case MyColumns.Size:
                    ...
                    //case MyColumns.Value3:
                    {
                        ...
                        //		item.data.size = EditorGUI.Slider(cellRect, GUIContent.none, item.data.size, 0f, 1f);
                        ...
                        //else
                        {
                            ...
                            if (column == MyColumns.Size)
                                value = Update.DownloaderComponent.GetDisplaySize (item.data.asset.len);
                            //if (column == MyColumns.Value2)
